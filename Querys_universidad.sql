...
select c.id as curs, asi.nombre as 'nombre asignatura', c.anyo_inicio, c.anyo_fin from curso_escolar c, asignatura asi, persona p where p.nif = '26902806M' order by c.id asc;
...
select departamento.nombre as departamento, grado.nombre as grado, asignatura.nombre from departamento inner join profesor on departamento.id = profesor.id_departamento inner join asignatura on profesor.id_profesor = asignatura.id_profesor inner join grado on asignatura.id_grado = grado.id where grado.id = 4;
#Retorna un llistat amb tots els alumnes que s'han matriculat en alguna assignatura durant el curs escolar 2018/2019.
select distinct persona.nombre, apellido1, apellido2, anyo_inicio, anyo_fin from persona inner join alumno_se_matricula_asignatura on persona.id = alumno_se_matricula_asignatura.id_alumno inner join curso_escolar on alumno_se_matricula_asignatura.id_curso_escolar = curso_escolar.id where curso_escolar.id = 5;
...
select persona.apellido1, persona.apellido2 , persona.nombre, departamento.nombre as departamento from persona left join profesor on persona.id = profesor.id_profesor left join departamento on profesor.id_departamento = departamento.id order by persona.apellido1, persona.apellido2, persona.nombre asc;
...
select asignatura.nombre as asignatura from asignatura left join profesor on asignatura.id_profesor = profesor.id_departamento left join persona on profesor.id_profesor = profesor.id_profesor where persona.apellido1 is null;
...
select anyo_inicio, count(persona.nombre) as numero_matriculats from curso_escolar left join alumno_se_matricula_asignatura ama on curso_escolar.id = ama.id_curso_escolar left join persona on ama.id_alumno = persona.id group by curso_escolar.anyo_inicio;
